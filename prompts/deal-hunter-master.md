...
1. Прочитай env-переменные:
   - `SOURCE_GROUP` (одно из: `baseline`, `A1`, `A2`, `A3`, `B`, `C`) — определяет какие источники проверять
   - `DEAL_HUNTER_HOME` — корень установки (по умолчанию `~/.claude`, может быть `~/Code/deal-hunter-with-claude` если запуск из репо)
2. Прочитай `$DEAL_HUNTER_HOME/data/schedule.yaml` чтобы получить:
...
3. Прочитай `$DEAL_HUNTER_HOME/data/landed_cost_table.yaml` для расчёта landed cost (только если SOURCE_GROUP=C).
4. Используй `python3 $DEAL_HUNTER_HOME/scripts/sheets_write.py --read --tab <Tab>` чтобы прочитать состояние Google Sheet.
5. Используй `python3 $DEAL_HUNTER_HOME/scripts/sheets_write.py --tab <Tab> --mode <append|upsert|mark_unavailable> --rows '<json>'` для записи.
6. Уведомления: `bash $DEAL_HUNTER_HOME/scripts/tg_notify.sh <HOT_DEAL|HELP_NEEDED|RUN_SUMMARY|STALE_QUEUE|YIELD_DROP|DIST_SHIFT> "<message>"`.
...
Используй `$DEAL_HUNTER_HOME/data/landed_cost_table.yaml` для расчёта landed cost — не угадывай курсы и пошлины.
...
Запись через `python3 $DEAL_HUNTER_HOME/scripts/sheets_write.py`.
...
* положи строку в `$DEAL_HUNTER_HOME/state/stash.jsonl` (sheets_write делает это автоматически при ошибке)
* запусти `bash $DEAL_HUNTER_HOME/scripts/tg_notify.sh HELP_NEEDED "Sheets API down, N rows in stash"`
...
2. Добавь строку в `Pending_Help_Queue`:
   ...
   ```
3. Шли `bash $DEAL_HUNTER_HOME/scripts/tg_notify.sh HELP_NEEDED "<инструкция>"` — указывай конкретно: «открой Claude Code, запусти `$DEAL_HUNTER_HOME/scripts/help-deals.sh`, в очереди N задач».
...
5. Для каждого источника группы:
   ...
   - Для каждого URL — fetcher по `source_endpoints[X].fetcher`:
     ...
     - `playwright` → `python3 $DEAL_HUNTER_HOME/scripts/fetch_kz.py "<url>"` (для anti-bot сайтов).
